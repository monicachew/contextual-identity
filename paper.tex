...
We consider it a contextual identity violation when multiple identities are
linked without user intent, or when the user cannot choose which contextual
identity to assert.
...
It is unlikely that the majority of users are consciously aware of having
multiple contextual identities. Imagine two users at the opposite ends of the
spectrum:
\begin{itemize}
\item A user who participates in no social networks, reads paper newspapers,
uses the Internet for well-defined tasks (e.g., coordinating visits with
friends over email, looking up restaurant reviews) but does not otherwise
engage in discretionary browsing.
\item A savvy user who is active on Facebook, Twitter, Instagram and has
different posting levels, topics, and followers on each, maintains multiple
email accounts and pseudonyms, is online multiple hours daily for both work and
personal reasons. 
\end{itemize}

The second user may be much more aware of contextual identities, but even the
first user would benefit from protection against contextual identity
violations. For example, engaging in a long-term task such as house-buying
...
\paragraph{Mitigating explicit unwanted linkage}
...
\paragraph{Remembering to Forget}
